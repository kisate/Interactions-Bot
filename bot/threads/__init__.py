    ...
    def run(self):
        with self.bot.lock:
            try:
                ...
                traceback.print_exc()
                self.bot.say(
                    'Exception occured while path-finding. Check console', 1)
                return
                ...
                for action in step[1]:
                    if action == 0:
                        self.bot.update_position(
                            [step[0][0]+0.5, step[0][1], step[0][2]+0.5])
                    elif action == 2:
                        self.bot.lock.release()
                        res = self.bot.mine_with_tool(step[0], setup=True)
                        self.bot.lock.acquire()
                        if not res or res == -1:
                            self.bot.say('Could not do action {} on block {}'.format(
                                action, step[0]), -1)
                            return
                    elif action == 3:
                        self.bot.lock.release()
                        res = self.bot.mine_with_tool(
                            np.sum([[0, 1, 0], step[0]], axis=0), setup=True)
                        self.bot.lock.acquire()
                        if not res or res == -1:
                            self.bot.say('Could not do action {} on block {}'.format(
                                action, step[0]), -1)
            ...
            self.bot.say(
                'Finished moving. My position is {}'.format(self.bot.position))
            return True
...
class FallingThread(Thread):
    ...
    def run(self):
        ...
        while self.bot.world.get_block(position[0], position[1] - 1, position[2]) == 0:
            position = [floor(x) for x in self.bot.position]
            self.bot.update_position(
                [position[0] + 0.5, position[1] - 1, position[2] + 0.5])
            sleep(0.1)
            if self.bot.break_event.isSet():
                self.bot.break_event.clear()
                self.bot.say('Stopped', 1)
...
class MultiMiningThread(Thread):
    ...
    def run(self):
        ...
        self.blocks.sort(key=lambda x: self.comparator(
            self.bot.position, x), reverse=True)
        ...
        while self.blocks:
            ...
            if self.with_tool:
                try:
                    ...
                    self.bot.say(
                        "No tool of {} type, aborting.".format(e.tool), 3)
                    print("No tool of {} type, aborting.".format(e.tool))
                    break
            else:
                thread = MoveToAndMineThread(
                    'mining', self.bot, self.args, with_tool=False)
...
class MoveToAndMineThread(Thread):
    ...
    def run(self, time_to_wait=0.4):
        ...
        thread = MovingThread('moving', self.bot, self.args,
                              time_to_wait=time_to_wait)
        thread.run()

        if sum((floor(self.bot.position[i]) + self.args['pivot'][i] - self.args['end'][i])**2 for i in range(3))**0.5 > self.bot.MINING_RADIUS:
            print("Distance to block is {}".format(sum((floor(
                self.bot.position[i]) + self.args['pivot'][i] - self.args['end'][i])**2 for i in range(3))**0.5))
            self.bot.say('Could not get to block {}. My position is {}'.format(
                self.args['end'], self.bot.position), 2)
            return False
    ...
    def run(self):
        ...
        while True:
            ...
            if sum([(target_pos[i] - self.bot.position[i])**2 for i in range(3)])**0.5 > 3:
                block_under_target = self.bot.world.get_block(
                    floor(target_pos[0]), floor(target_pos[1]) - 1, floor(target_pos[2]))
                if block_under_target != 0 and block_under_target not in self.bot.world.info['blocks']['damaging']:
                    args = {'start': [floor(x) for x in self.bot.position], 'end': [
                        floor(x) for x in target_pos], 'radius': 3}
...
class GetToChunkThread(Thread):
    ...
    def run(self):
        try:
            _ = self.bot.world.get_chunk_by_chunk_coords(*self.target)
            ...
            distance_to_chunks = [
                [(x[0]-self.bot.position[0])**2 + (x[1]-self.bot.position[2])**2] for x in nearest_chunks]
            nearest_chunk_coords = nearest_chunks[distance_to_chunks.index(
                min(distance_to_chunks))]
            thread = GetToChunkThread(
                self.name, self.bot, self.args, nearest_chunk_coords)
            thread.run()
        for x in range(16):
            for z in range(16):
                self.args['start'] = [floor(x) for x in self.bot.position]
                self.args['end'] = [self.target[0] *
                                    16 + x, 0, self.target[1]*16 + z]
